...
## Resolving price disagreements: the resolution ladder

When sources disagree about one record's price on one unit, they aren't competing offers, they're competing reports of a single fact, and the shipped value is picked by a fixed ladder of rungs (`rates._resolution`), each consulted in order. A rung partitions the field of candidates and keeps its best class: several left standing restricts the field for the rungs below, and a rung whose evidence can't rank every candidate in the field is skipped whole, because unknown is never zero: a source with no measured wrongness isn't 0% wrong, it's unmeasured, and may not be outranked on evidence nobody gathered. The ladder ends when one candidate stands, and the note's `resolved_by` names the rung that decided.

1. **Origin.** A source that is first-party for the record's provider (the horse's mouth speaking about its own product) ends the contest, in either direction: a witness claiming the provider charges less than the provider says loses the same as one claiming more. Origin status is a property of a (source, record) pair, not a source: OpenRouter's API is first-party for OpenRouter's own resale rows and a witness everywhere else, today's one origin pairing. Two first-party sources disagreeing (a provider's page lagging its API) restrict the field to themselves and the rungs below separate them.
2. **Freshness.** Whose underlying data for this record changed most recently, read from each source's commit history: source-wide for the single-bulk-file sources (LiteLLM, genai-prices), per-record for models.dev's per-model files, checked lazily and only for contested records. Freshness outranks any declared standing among witnesses because a price change is an event only fresh witnesses have seen. That stopped being academic on 2026-08-24: OpenAI cut GPT-5.6 Sol's price, LiteLLM's community shipped the new number within two days, and models.dev's per-model file still hadn't moved; a standing-first rule ships the superseded price with a straight face precisely when a price just moved.
3. **Corroboration.** Among the tied, the value independent witnesses agree on, counting distinct upstream groups rather than raw sources, so two feeds syncing from one catalog can't vote twice. Inert until each source's upstream graph is declared and verified in its scorecard; agreement among sources whose upstreams are unknown is not evidence, it may be one origin counted twice.
4. **Preferred.** Pinned overrides only, each with its reason recorded: a source deliberately promoted or demoted. Nothing is hand-ranked here by default; the measured rungs below do the ranking work, so this rung separates nothing in the normal state.
5. **Accuracy.** Lowest measured contradiction rate against origin checks, dated evidence over a window counted in check runs rather than calendar days (a calendar window is a domain-specific constant in disguise). Inert until the origin-check contradiction ledger exists.
6. **Coverage.** Fraction of the build's input records the source carries, computed per run before any merge. A late separator only, never a proxy for accuracy.
7. **Registry order.** The declared strict total order over sources, the determinism floor: every source holds a unique rank by construction, "preferred" as a badge several sources share is unrepresentable, so the ladder can never tie and every note can always name its rung.

**Every note describes the shipped value.** A contested unit runs the ladder over all its carriers at once, one selection, not pairwise matches, and writes one note per carrier still past the 2% threshold from the shipped value: every note's `chosen_value` is the number on the label, however many sources disagreed. (The pairwise design this replaced resolved each fallback against the spine independently, so a later winner could overwrite an earlier one's price while the earlier note kept claiming a value that never shipped.)

**The ledger is self-describing.** The envelope's `resolution` object carries the ladder and each source's scorecard as consulted for that build (origin pairings, upstream declarations, overrides with reasons, wrongness, coverage, registry rank), so any record's resolution is replayable from the ledger file alone and a note's `resolved_by` can always be audited against the standing that produced it.

**Never blocks a result.** A `live()` caller whose network can't reach a freshness check doesn't fail the whole call over one rung's evidence: the rung skips and the ladder continues, the same "a quality signal failing costs nothing but that signal" pattern the stable tier follows for its own freshness check.

**Price is never a rung.** Choosing among distinct competing offers by price is the caller's query (`--sort-by price.input_mtok`, over separate per-provider rows); choosing among conflicting reports of one fact by which number is most flattering would bias the registry toward understating bills. The ladder adjudicates testimony; the query layer compares offers.

**Caching.** Each source's last-known commit date is cached for an hour, so repeat calls and repeat records within one fusion run don't refetch. Confirmed live (2026-08-24): the commit-history feed used for freshness carries none of `api.github.com`'s `x-ratelimit-*` headers, it's served from a separate, undocumented budget on GitHub's web frontend, not the REST API's 60/hour unauthenticated or 5,000/hour token-authenticated core limit that `stable` and `live` already use. Caching is the mitigation regardless of where that undocumented ceiling falls.
...
**A scheduled CI job republishes the ledger weekly.** A GitHub Actions workflow re-runs the full fusion every Monday at 06:00 UTC (and on demand): it rebuilds the bundled ledger from the four sources, runs the schema-drift probes and the full test suite against the result, commits the refreshed snapshot, and publishes it as a dated GitHub Release. So `stable` has a fresh snapshot to fetch on a predictable cadence, and a source that changes shape trips a probe within the week rather than degrading `live` unnoticed.
